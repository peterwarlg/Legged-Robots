...
class HipRobot:
    def __init__(self):
        ...
        self.k_leg_p: float = 2.0  # 腿部控制时的kp
        self.k_leg_v: float = 0.1  # 腿部控制时的kv
        ...
        # x y z
        self.workPoint_B: np.ndarray = np.array([0.0, 0.0, 0.0])
        self.workPoint_H: np.ndarray = np.array([0.0, 0.0, 0.0])

        self.workPoint_B_desire: np.ndarray = np.array([0.0, 0.0, 0.0])
        self.workPoint_H_desire: np.ndarray = np.array([0.0, 0.0, 0.0])
    ...
    def update_robot_state(self):
        self.system_ms = self.system_ms + float(self.time_step)

        self.is_foot_touching_flg = self.is_foot_touching()

        now_imu_angles = self.get_IMU_Angle()
        now_imu_angles_dot = np.array([0.0, 0.0, 0.0])
        for i in range(3):
            now_imu_angles_dot[i] = (now_imu_angles[i] - self.eulerAngle[i]) / (0.001 * float(self.time_step))
            self.eulerAngle_dot[i] = now_imu_angles_dot[i]
            self.eulerAngle[i] = now_imu_angles[i]

        r = R.from_euler('xzy', now_imu_angles, degrees=True)
    ...
    def update_xz_dot(self):
        ...
        now_x_dot = - (now_x - pre_x) / (0.001 * float(self.time_step))
        now_z_dot = - (now_z - pre_z) / (0.001 * float(self.time_step))

        now_x_dot = (now_x_dot + self.pre_x_dot) * 0.5
        now_z_dot = (now_z_dot + self.pre_z_dot) * 0.5
