...
class RobotStatus:
    def __init__(self):
        ...
        self.is_foot_touching_flg_RR: bool = True
        ...
        self.pre_is_foot_touching_flg_RR = False
...
class Devices:
    def __init__(self, _robot: Robot) -> None:
        ...
        # RR leg的硬件注册
        self.spring_motor_RR: Motor = _robot.getMotor("Spring linear motor RR")
        self.spring_pos_sensor_RR: PositionSensor = _robot.getPositionSensor("Spring position sensor RR")
        self.touch_sensor_RR: TouchSensor = _robot.getTouchSensor("touch sensor RR")
        self.X_motor_RR: Motor = _robot.getMotor("X rotational motor RR")
        self.X_motor_position_sensor_RR: PositionSensor = _robot.getPositionSensor("X position sensor RR")
        self.Z_motor_RR: Motor = _robot.getMotor("Z rotational motor RR")
        self.Z_motor_position_sensor_RR: PositionSensor = _robot.getPositionSensor("Z position sensor RR")
        # setposition 朝上为+
        self.shorten_motor_RR: Motor = _robot.getMotor('Shorten linear motor RR')
        self.shorten_position_sensor_RR: PositionSensor = _robot.getPositionSensor("Shorten position sensor RR")
        ...
        # Leg RR enable
        self.spring_pos_sensor_RR.enable(time_step)
        self.X_motor_position_sensor_RR.enable(time_step)
        self.Z_motor_position_sensor_RR.enable(time_step)
        self.touch_sensor_RR.enable(time_step)
        self.shorten_position_sensor_RR.enable(time_step)


    def set_spring_force(self, spring_motor: Motor, force: float):
        spring_motor.setForce(-force)

    def get_spring_length_RR(self) -> float:
        l: float = self.spring_pos_sensor_RR.getValue()
        return 0.8 - l

    def set_X_torque_RR(self, torque):
        self.X_motor_RR.setTorque(torque)

    def set_Z_torque_RR(self, torque):
        self.Z_motor_RR.setTorque(torque)

    def get_X_motor_angle_RR(self) -> float:
        angle: float = self.X_motor_position_sensor_RR.getValue()
        return angle * 180.0 / math.pi

    def get_Z_motor_angle_RR(self) -> float:
        angle: float = self.Z_motor_position_sensor_RR.getValue()
        return angle * 180.0 / math.pi

    def is_foot_touching_RR(self) -> bool:
        return self.touch_sensor_RR.getValue()
    ...
    def set_shorten_length_RR(self, length: float):
        self.shorten_motor_RR.setPosition(length)

    def get_shorten_length_RR(self) -> float:
        return self.shorten_position_sensor_RR.getValue()

    def fix_motor_angle_RR(self):
        x = self.X_motor_position_sensor_RR.getValue()
        z = self.Z_motor_position_sensor_RR.getValue()
        self.X_motor_RR.setPosition(x)
        self.Z_motor_RR.setPosition(z)
